...
export function detectMissingFields(templateKey, session) {
  ...
  if (templateKey === 'level4') {
    ...
    missing.push({ id: 'warranty',       label: 'Warranty status (discussed with customer?)' });
...
export function buildMissingInfoEmail(missingFields, session) {
  const model    = (session?.model || session?.device || '').toUpperCase() || 'your ScanSnap scanner';
  const caseNum  = session?.caseNumber ? ` [Case ${session.caseNumber}]` : '';

  const fieldList = missingFields.map(f => `- ${f.label}`).join('\n');

  return `Dear Customer,

Thank you for your continued patience while we work to resolve the issue with your ${model}.${caseNum}

To proceed with the next steps, we require the following additional information:

${fieldList}

Please reply directly to this email so no duplicate case is created. If you contact us by phone, please mention your case number so we can locate the existing case immediately.
